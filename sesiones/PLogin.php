...
class Login
{
    ...
    private function dologinWithPostData()
    {
        ...
        if (empty($_POST['username'])) {
            $this->errors[] = "USERNAME está vacío, .";
        } elseif (empty($_POST['password'])) {
            $this->errors[] = "Campo Password está vacío.";
        } elseif (empty($_POST['username']) && empty($_POST['password'])) {
          $this->errors[] = "Todos los campos estan vacíos.";
        } elseif (!empty($_POST['username']) && !empty($_POST['password'])){
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $user_password = $this->db_connection->real_escape_string($_POST['password']);




                #$user_key = $this->db_connection->real_escape_string(base64_encode($_POST['user_key']));

                // database query, getting all the info of the selected user (allows login via email address in the
                // username field)
                $sql = "SELECT *
                        FROM usuarios
                        WHERE usuarionameUsuario = '". $user_name. "' ;";
                ...
                if ($result_of_login_check->num_rows == 1) {
                  ...
                  $result_row = $result_of_login_check->fetch_object();
                    ...
                    $DBPassword = $result_row->usuario_password;

                    if ($user_password == $DBPassword) {
                      //Variable de Status en la BD
                      $DBStatus = $result_row->usuario_status;
                      ...
                      if ($DBStatus == 1) {
                            ...
                            $_SESSION['user_name'] = $result_row->usuario_name;
